...
public class GraphBuilder {
	public MBTAgraph loadAPIFile(MBTAgraph g) throws Exception {
		// read json string from file
//		BufferedReader reader = new BufferedReader(new FileReader(
//				"AllRoutes.txt"));
		InputStream file = this.getClass().getClassLoader().getResourceAsStream("AllRoutes.txt");
		...
		for (ModeEntity modeFromAllRoute : allRoutes.getMode()) {
			if (modeFromAllRoute.getRoute_type().equals("0")
				...
				for (RouteEntity routeFromAllRoute : modeFromAllRoute
						.getRoute()) {
					InputStream file1 = this.getClass().getClassLoader().getResourceAsStream(routeFromAllRoute.getRoute_id() + ".txt");
