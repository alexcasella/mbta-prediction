...
public class HeuristicSearch {
	...
	public ArrayList<TransferInfo> getTransferInfo() {
		ArrayList<TransferInfo> transInfo = new ArrayList<>();
		for(int i = 1; i < shortestPath.size() - 1; i++) {
			PathInfo current = shortestPath.get(i);
			PathInfo next = shortestPath.get(i + 1);
			
			if(current.getRoute_id().equals("Walk")) {
				TransferInfo oneTrans = new TransferInfo();
				oneTrans.setStop_id(current.getStop_id());
				oneTrans.setRoute_id(next.getRoute_id());
				oneTrans.setStop_name(current.getStop_name());
				oneTrans.setDirection_id(next.getDirection_id());
				transInfo.add(oneTrans);
			}
		}
		return transInfo;
	}
