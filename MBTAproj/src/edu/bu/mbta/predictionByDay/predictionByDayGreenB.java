...
public class predictionByDayGreenB extends Thread{

	AmazonDynamoDBClient dynamoDB;
	public predictionByDayGreenB(AmazonDynamoDBClient DB) {
		dynamoDB = DB;
	}
	...
	private void execute(AmazonDynamoDBClient dynamoDB) throws Exception {
		...
		String tableName = "PredictionsByStop_GreenB_"+month + "."+ day +"."+year;		
		createTable.create(dynamoDB, tableName);
		while (true) {
			predictionByDate.update(dynamoDB, tableName, "Green-B");
