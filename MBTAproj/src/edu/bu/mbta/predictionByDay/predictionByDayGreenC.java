...
public class predictionByDayGreenC extends Thread{

	AmazonDynamoDBClient dynamoDB;
	public predictionByDayGreenC(AmazonDynamoDBClient DB) {
		dynamoDB = DB;
	}
	...
	private void execute(AmazonDynamoDBClient dynamoDB) throws Exception {
		...
		String tableName = "PredictionsByStop_GreenC_"+month + "."+ day +"."+year;		
		createTable.create(dynamoDB, tableName);
		while (true) {
			predictionByDate.update(dynamoDB, tableName, "Green-C");
