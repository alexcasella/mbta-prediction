...
public class predictionByDayGreenE extends Thread{
	
	AmazonDynamoDBClient dynamoDB;
	public predictionByDayGreenE(AmazonDynamoDBClient DB) {
		dynamoDB = DB;
	}
	...
	private void execute(AmazonDynamoDBClient dynamoDB) throws Exception {
		...
		String tableName = "PredictionsByStop_GreenE_"+month + "."+ day +"."+year;		
		createTable.create(dynamoDB, tableName);
		while (true) {
			predictionByDate.update(dynamoDB, tableName, "Green-E");
