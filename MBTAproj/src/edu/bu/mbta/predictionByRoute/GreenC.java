...
public class GreenC extends Thread{
	
	AmazonDynamoDBClient dynamoDB;
	public GreenC(AmazonDynamoDBClient DB) {
		dynamoDB = DB;
	}
	...
	private void execute(AmazonDynamoDBClient dynamoDB) throws Exception {
		String tableName = "PredictionsByRoute_GreenC";
		createTable.create(dynamoDB, tableName);
		while (true) {
			updatePrediction.update(dynamoDB, tableName, "Green-C");
